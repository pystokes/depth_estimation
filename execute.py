...
import os
os.environ['DepthEstimation_HOME'] = os.path.dirname(os.path.abspath(__file__))
DepthEstimation_HOME = os.environ['DepthEstimation_HOME']
...
from model.model import DepthEstimation
from utils.common import CommonUtils

# Set logger
log_date = datetime.datetime.today().strftime('%Y%m%d')
log_path = Path(DepthEstimation_HOME).joinpath(f'log/{log_date}.log')
...
class Executor(object):
    ...
    def __init__(self, exec_type, config=None, y_dir=None):
        ...
        if self.exec_type == 'preprocess':
            if self.config.preprocess.save_dir:
                ...
                self.save_dir = Path(DepthEstimation_HOME).joinpath('data_storage', issue_id)
        elif self.exec_type == 'train':
            if self.config.train.save_dir:
                self.save_dir = Path(self.config.train.save_dir)
            else:
                self.save_dir = Path(DepthEstimation_HOME).joinpath('results', 'train', issue_id)
        else:
            if y_dir:
                self.save_dir = Path(y_dir)
            else:
                self.save_dir = Path(DepthEstimation_HOME).joinpath('results', 'detect', issue_id)
    ...
    def load_model(self, gpu_id=None):
        ...
        model = DepthEstimation(self.exec_type, self.config)
        # Multi-GPU mode
        if on_multi_gpu:
    ...
    def train(self, model, device):
        ...
        create_loaders = CreateDataLoader()
        train_loader, valid_loader = create_loaders.build_for_train(self.config)
    ...
    def detect(self, trained_model, device, x_dir, y_dir):
        ...
        create_loaders = CreateDataLoader()
        data_loader = create_loaders.build_for_detect(self.config, x_dir)
...
if __name__ == '__main__':
    ...
    logger.info(f'Begin DepthEstimation in {args.exec_type.upper()} mode')
